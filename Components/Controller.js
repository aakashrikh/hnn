...
const Controller = (props) => {
  ...
  const renderPlayPauseButton = () => {
    switch (isPlaying) {
      case 'playing':
        return <Fontisto name="pause-circle" size={height / 10.8} color="white" />;
      case 'paused':
        return <Fontisto name="play-circle" size={height / 10.8} color="white" />;
      default:
        return <ActivityIndicator size={height / 15} color="gray" />;
    }
  };
  ...
  return (
    <View style={styles.screen}>
      <TouchableOpacity onPress={props.goPrev} style={{marginTop:22}}>
        <AntDesign name="play-skip-back" size={height / 25} color="white" />
      </TouchableOpacity>
      <TouchableOpacity onPress={onPlayPause}>
        {renderPlayPauseButton()}
      </TouchableOpacity>
      <TouchableOpacity onPress={props.goNext} style={{marginTop:22}}>
        <AntDesign name="play-skip-forward" size={height / 25} color="white" />
      </TouchableOpacity>
    </View>
  );
};
