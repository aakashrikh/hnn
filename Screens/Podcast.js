    ...
    if(isLoading)
    {
      ...
      let podcasts = list.map((pd,id) => {
       pod_data.push(new Song(
        ...
        return(
          <ListItem containerStyle={{backgroundColor:'#000000',color:'#eee'}} onPress={()=>this.props.navigation.navigate('SongsPlay',{
            itemId: pd.id,son_data:pod_data})} >

          <Avatar  source={{uri: global.uri+pd.podcast_pic}} />
            <ListItem.Content> 
              <ListItem.Title style={{color:'#eee'}}>{pd.podcast_name}</ListItem.Title>
              <View style={styles.subtitleView}>
                <Text style={{color:'#eee'}}>{pd.short_description}</Text>
              </View>
            </ListItem.Content>
